...
export default function NonNegotiablesPage() {
    ...
    const toggleMutation = useMutation({
        ...
        onSuccess: (data, variables) => {
            ...
            if (isToday) {
                 ...
                 const isNowDone = updatedItem.completedDates.some((d: string) => new Date(d).toDateString() === today.toDateString());
                 
                 // If we just completed it, show a motivational toast
                 if (isNowDone) {
                     // Check if this was the LAST one
                     const otherItems = items.filter((i: NonNegotiable) => i.id !== variables.id);
                     const allOthersDone = otherItems.every((i: NonNegotiable) => i.completedDates.some((d: string) => new Date(d).toDateString() === today.toDateString()));
                     
                     if (allOthersDone && otherItems.length > 0) {
                        toast("🔥 PROTOCOL COMPLETE", {
                            description: "You have conquered the day. Momentum is building.",
                            className: "bg-emerald-500 border-emerald-600 text-white font-bold",
                            duration: 5000,
                        })
                     } else {
    ...
    const isCompletedToday = (item: NonNegotiable) => {
        ...
        return item.completedDates.some(d => new Date(d).toDateString() === today)
    }
        ...
        <div className="min-h-screen bg-black text-white font-sans selection:bg-emerald-500/30">
            ...
            <div className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-white/5">
                <div className="max-w-3xl mx-auto px-6 py-4 flex items-center gap-4">
                    <Link href="/dashboard" className="p-2 rounded-full hover:bg-white/10 transition-colors text-zinc-400 hover:text-white">
            ...
            <main className="max-w-3xl mx-auto px-6 py-8 space-y-12">
                ...
                <section className="space-y-6">
                     <div className="flex items-center justify-between">
                        ...
                        <span className="text-zinc-500 text-sm font-mono">{new Date().toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })}</span>
                    </div>
                        ...
                        <div className="grid gap-4">
                            {items.map((item: NonNegotiable) => (
                                <PremiumTaskCard 
                                    ...
                                    onToggle={() => toggleMutation.mutate({ id: item.id, date: today })}
                            ...
                            {items.length === 0 && (
                                <div className="text-center py-20 border-2 border-dashed border-zinc-900 rounded-3xl">
                                    ...
                                    <p className="text-zinc-600 text-sm mt-2">Discipline = Freedom</p>
                                </div>
                            )}
                        </div>
                    )}
