...
import { Loader2, Plus, Search, Calendar, Star, Trash2, ArrowLeft } from "lucide-react";
        ...
        <div className="relative z-10 max-w-5xl mx-auto space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                    ...
                    <p className="text-zinc-500 text-sm pl-10">Capture your journey, one day at a time.</p>
                </div>
                <Link href="/logs/new">
            ...
            <div className="flex flex-wrap items-center gap-3 bg-white/5 p-2 rounded-xl border border-white/5">
                ...
                <div className="flex items-center gap-2 px-2">
                    ...
                    />
                    {dateFilter && (
                        <button onClick={() => setDateFilter("")} className="text-xs text-red-400 hover:text-red-300 ml-1">Clear</button>
                    )}
                </div>
                <div className="h-6 w-px bg-white/10 mx-2" />
                <Button 
                    ...
                    className={cn("gap-2 hover:bg-white/5", showFavorites ? "text-yellow-400 hover:text-yellow-300" : "text-zinc-500")}
                >
            ...
            <div className="space-y-4">
                {isLoading ? (
                    ...
                    <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/5 border-dashed">
                        <p className="text-zinc-500">No logs found matching your criteria.</p>
                    </div>
                ) : (
                            ...
                            <div className="flex justify-between items-start gap-4">
                                <div className="space-y-2 flex-1">
                                    <div className="flex items-center gap-3">
                                        <h3 className="text-xl font-semibold text-zinc-100 group-hover:text-white transition-colors">{log.title}</h3>
                                        {/* Date Badge */}
                                ...
                                <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                     <Button
                                        ...
                                        onClick={(e) => {
                                            e.stopPropagation();
                                            toggleFavorite({ id: log.id, isFavorite: !log.isFavorite });
                                        }}
                                    >
                                        <Star className={cn("h-4 w-4", log.isFavorite && "fill-yellow-400 text-yellow-400")} />
                                    </Button>
                                    <Button
                                        size="icon"
                                        variant="ghost"
