...
function NewLogContent() {
  ...
  // Pre-fetch logic for Edit Mode (Dirty fix: using a direct fetch here inside generic "logs" query?)
  // Actually, let's use the list query but we might need to paginate.
  // For now, I will skip pre-filling until I fix the API in next step.
  // BUT I will implement the UI for it.
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <span className="text-zinc-500 text-sm">
                        {editId ? "Editing Entry" : format(new Date(date), "EEEE, MMMM d")}
                    </span>
                </div>
            </div>
                         ...
                         <div className="flex gap-2 flex-wrap items-center">
                            ...
                            <Popover open={isCategoryPopoverOpen} onOpenChange={setIsCategoryPopoverOpen}>
                                <PopoverTrigger asChild>
                                    <button type="button" className="flex items-center gap-1 px-3 py-1.5 rounded-full border border-dashed border-white/20 text-sm text-zinc-500 hover:text-white hover:border-white/40 transition-colors">
                                        <Plus className="h-3 w-3" />
                                        Category
                                    </button>
                                </PopoverTrigger>
                                <PopoverContent className="w-64 p-3 border-white/10 bg-zinc-900" align="start">
