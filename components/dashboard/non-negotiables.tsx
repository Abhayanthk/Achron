...
export function NonNegotiables() {
  ...
  const completedCount = items.filter((i: NonNegotiable) => 
      i.completedDates.some(d => new Date(d).toDateString() === today.toDateString())
  ).length
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-xs text-zinc-500 mb-2">
        <Flame className={cn("size-4", completedCount === total && total > 0 ? "text-emerald-500" : "text-orange-500")} fill="currentColor" />
        <span>{isLoading ? "Loading..." : `${completedCount}/${total} Completed Today`}</span>
      </div>

      <div className="space-y-2">
        {items.slice(0, 3).map((item: NonNegotiable) => {
           const isCompleted = item.completedDates.some(d => new Date(d).toDateString() === today.toDateString())
           return (
              <div
                key={item.id}
              ...
              >
                <span className={cn("truncate max-w-[150px]", isCompleted ? "text-zinc-500 line-through" : "text-zinc-200")}>
                  {item.title}
                </span>
                <button
                  onClick={() => toggleMutation.mutate({ id: item.id, date: today })}
                  className={cn(
