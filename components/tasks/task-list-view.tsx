...
import { Loader2, Calendar as CalendarIcon, CheckCircle2, Circle, Flag } from "lucide-react";
...
export function TaskListView({ filter, searchQuery }: TaskListViewProps) {
  ...
  const filteredTasks = tasks?.filter((t: Task) => {
      ...
      switch (filter) {
          ...
          case "UPCOMING":
              return t.dueDate && isFuture(new Date(t.dueDate)) && !isSameDay(new Date(t.dueDate), today);
          case "OVERDUE":
              return t.dueDate && isPast(new Date(t.dueDate)) && !isSameDay(new Date(t.dueDate), today);
          case "ALL":
          default:
              return !t.isCompleted;
      }
  ...
  filteredTasks.sort((a: Task, b: Task) => {
      const pDiff = PRIORITY_ORDER[a.priority || "WHITE"] - PRIORITY_ORDER[b.priority || "WHITE"];
        ...
        {filteredTasks.map((task: Task) => (
            <motion.div
                ...
                className={cn(
                    ...
                    PRIORITY_COLORS[task.priority || "WHITE"].split(" ")[1].replace("border-", "hover:border-") // Subtle hover effect matching priority
                )}
            >
                {/* Priority Indicator Strip */}
                <div className={cn("absolute left-0 top-0 bottom-0 w-1", PRIORITY_COLORS[task.priority || "WHITE"].split(" ")[2].replace("bg-", "bg-"))} />
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-1">
                    <div className="flex items-center gap-2 ">
                        <span className={cn("text-sm font-medium leading-tight truncate text-zinc-100", task.isCompleted && "line-through text-zinc-500 opacity-50")}>
                            {task.title}
                        </span>
                    ...
                    <div className="flex items-center gap-3 text-xs text-zinc-500">
                        ...
                        <div className={cn("flex items-center gap-1", PRIORITY_COLORS[task.priority || "WHITE"].split(" ")[0])}>
